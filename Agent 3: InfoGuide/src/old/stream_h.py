...
class MTSS_Copilot:
    ...
    @staticmethod
    def simulate_QA_agent_turn(user_role, user_query, data, conversation_history=""):
        llm_response = None
        tokenizer, model, id2label = MTSS_Copilot.load_anomaly_prediction_model()

        context = conversation_history

        print("this is the conversation-history: ", conversation_history)

        if user_role == 'Anomaly Prediction and Sensor Values':
            time_series_data = ["[663. 463. 500.]"]
            print(MTSS_Copilot.get_anomaly_prediction_context(tokenizer, model, id2label, user_query, time_series_data))
            context += str(
                MTSS_Copilot.get_anomaly_prediction_context(tokenizer, model, id2label, user_query, time_series_data))
        else:
    ...
    @staticmethod
    def run_demo(user_role, user_query, conversation_history=""):
...
def main():
    ...
    print("this is the sleected-role: ", selected_role)
    ...
    if st.button("Submit"):
        if user_query:
            interaction = MTSS_Copilot.run_demo(selected_role, user_query, st.session_state.conversation_history)
